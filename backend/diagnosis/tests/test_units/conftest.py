...
def mock_ner_model():
    """Мок NER модели."""
    with patch("ml.nlp.extractors.ner_rubio_finetuned.AutoTokenizer") as MockTokenizer, \
...
def mock_hybrid_components(rule_based_extractor, semantic_extractor, ner_extractor):
    ...
    with patch("ml.nlp.extractors.hybrid.RuleBasedExtractor") as MockRule, \
        ...
        yield {
            'rule': rule_based_extractor,
            'ner': ner_extractor,
            'semantic': semantic_extractor
        }
...
def hybrid_extractor(mock_hybrid_components):
    ...
    with patch("ml.nlp.extractors.hybrid.connection") as mock_conn, \
...
def mock_nlp_loaded():
    ...
    with patch("diagnosis.views.nlp_loaded_successfully", True), \
         patch("diagnosis.views.hybrid_extractor", Mock()):
        yield
...
def mock_django_dependencies():
    ...
    with patch("diagnosis.views.Symptom") as mock_symptom, \
        ...
        mock_symptom_manager.all.return_value.order_by.return_value.values_list.return_value = ["головная боль", "кашель"]
        
        yield {
            "mock_hybrid_extractor": mock_hybrid_extractor,
            "mock_model": mock_model,
            "mock_symptom": mock_symptom
        }