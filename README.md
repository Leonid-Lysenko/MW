...
- **Гибридный подход**: Rule-based + NER (RuBioRoBERTa) + семантический поиск
- **Rule-based словарь**: 2034 фразы-синонима для 304 симптомов
- **NER-модель**: RuBioRoBERTa, дообученная на 3630 фразах (F1 = 0.931)
- **Семантический поиск**: multilingual-E5-large + pgvector
- **Итоговое качество гибрида**: Weighted F1 = 0.761
...
### Уровни тестирования:
   ...
   - 114 unit-тестов - проверка отдельных компонентов
   - 43 интеграционных тестов - валидация взаимодействия модулей
   - 4 end-to-end теста - тестирование полных сценариев

Общий процент успешного прохождения тестов составляет 100%. Покрытие кода тестами достигает 90%.
...
│       ├── extractors/     # Экстракторы симптомов

│       ├── data/           # Датасеты и словари синонимов

│       └── models/         # Дообученные NER-модели
...
Для более полного ознакомления предлагается изучение следующих документов в **MedWebsite/docs/**:
   ...
   - NLP_MODULE.md - документация гибридного NLP-модуля
